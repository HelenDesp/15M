...
export default function NFTViewer() {
  ...
  const fetchNFTs = async () => {
    ...
    try {
      ...
      console.log("Moralis result:", data.result);

      const items = await Promise.all(
        data.result.map(async (nft) => {
          console.log("NFT raw:", nft);
          ...
          try {
            if (nft.metadata) {
              ...
              console.log("Fetching token_uri:", nft.token_uri);
          ...
          console.log("Parsed metadata image:", image);
      ...
      setNfts(items); // not filtering by .image to debug missing ones
    } catch (err) {
